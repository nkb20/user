...
const handleUserRegistration = async function (req, res, next) {
  ...
  if (values.includes(new_user.role)) {
    role = new_user.role;
  }
  ...
  try {
    ...
    log.debug(`User registered with userId ${user.user_id}`)
...
const handleUserLogin = async (req, res, next) => {
  ...
  try {
    ...
    if (!data.userName || !data.password) {
      throw new InvalidInputException("All fields are required", 400)
    }

    const user = await user_model.findOne({ user_name: data.userName });
    ...
    if (isPassword) {
      ...
      res.status(201).json(JsonResponse.success("Token generated succesfully", { token: generatedToken, expires_in: tokenExpireTime(expTime) }))

    }
    else {
      res.status(400).json(JsonResponse.error("Enter correct password"))
      return;
    }
  }
  catch (exception) {
...
const handleChangePassword = async (req, res, next) => {
  ...
  try {
    ...
    await user_model.findOneAndUpdate(req.user_name, { password: encrypted_password })
...
const handleForgotPassword = async (req, res, next) => {
  ...
  if (fetchUUIddetails) {
    ...
    try {
      ...
      const encrypted_password = await bcrypt.hash(data.new_password, 10);
      ...
      if (exception instanceof InvalidInputException) {
        next(new InvalidInputException(err.message, 400));
        log.debug(exception)
        return;
      }
      else {
